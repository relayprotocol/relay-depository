...
contract RelayDepository is Ownable, EIP712 {
    ...
    function _hashCallRequest(
        ...
        // Get the EIP-712 hash with fixed chainId
        eip712Hash = _hashTypedDataWithChainId(structHash, 728126428);
    }

    /// @notice Custom implementation of _hashTypedData with fixed chainId
    /// @param structHash The struct hash to use in the EIP-712 hash
    /// @param chainId The fixed chainId to use
    /// @return digest The EIP-712 hash
    function _hashTypedDataWithChainId(bytes32 structHash, uint256 chainId) internal view returns (bytes32 digest) {
        ...
        // Compute the digest according to EIP-712
        digest = keccak256(
            abi.encodePacked(
                "\x19\x01",
                domainSeparator,
